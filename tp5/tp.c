...
void userland()
{
    ...
    // Q1: Déclenchez l'interruption "48" de manière logicielle.
	char * chaine = "bonsoir";
    asm volatile("int $48"::"a"(0xCAFE), "b"(chaine)); ; // -> GP si IDT non définie
	
	// Q5: Lecture de la mémoire du noyau :
    asm volatile("int $48"::"a"(GDT[GDT_C0].g)); // Lecture de la GDT depuis userland()
...
/* Q4: Afficher une chaîne de caractères dont l'adresse se trouve dans "ESI".
Essayez cet appel système depuis votre fonction userland(). */
...
void init_user_idt() // Q2: Appel de l'interruption "48" en ring 3
{
	idt_reg_t idtr;	 // IDTR
	...
	// Q3: Définit syscall_isr() tel que gestionnaire d'interruption n°48:
